...
export default function TrainingPlan({
  ...
  const getWeekData = (weekNum: number): PlanWeek => {
    return (
      TRAINING_PLAN_10K.find((w) => w.weekNum === weekNum) ||
      TRAINING_PLAN_10K[0]
    );
  };
  ...
  const findLoggedRun = (weekNum: number, dayNum: number): Run | undefined => {
    return runs.find(
      (run) => run.plan_week === weekNum && run.plan_day === dayNum,
    );
  };
  ...
  const getWorkoutTypeLabel = (type: string) => {
    switch (type) {
      case "interval":
        return "Trote (Intervalos)";
      case "rucking":
        return "Rucking (Carga)";
      case "fondo":
        return "Trote (Fondo)";
      default:
        return "Trote";
        ...
        <div className={styles.navHeader}>
          <div>
            ...
            <p className={styles.navSubtitle}>
              Completa 3 sesiones semanales para entrenar tu base aeróbica
            </p>
          </div>

          <button
            className={
              selectedWeekNum === activeWeekNum
                ? `${styles.activeWeekBtn} ${styles.currentWeekBadge}`
        ...
        <div className={styles.weekButtonsContainer}>
          {TRAINING_PLAN_10K.map((week) => {
            ...
            const completedCount = week.workouts.filter((w) =>
              findLoggedRun(week.weekNum, w.dayNum),
            ).length;
            ...
            >
              <div className={styles.cardHeader}>
                <div className={styles.headerTitles}>
                  ...
                  <span
                    className={`${styles.typeBadge} ${getWorkoutTypeColorClass(workout.workoutType)}`}
                  >
                    {getWorkoutTypeLabel(workout.workoutType)}
                  </span>
                </div>
                {completedRun && (
                  <span className={styles.checkmarkIcon}>✓ Completado</span>
                )}
              </div>
              ...
              <div className={styles.cardBody}>
                ...
                <div className={styles.workoutGoal}>
                  ...
                  <span>
                    Objetivo:{" "}
                    {workout.duration > 0 ? `${workout.duration} minutos` : ""}
                  ...
                  <div className={styles.completionStats}>
                    <div className={styles.statLine}>
                      <span>
                        ...
                        {completedRun.distance !== null &&
                        completedRun.distance !== undefined
                          ? `${completedRun.distance} km `
                          : ""}
                        {completedRun.rucking_weight !== null &&
                        completedRun.rucking_weight !== undefined
                          ? `(Carga: ${completedRun.rucking_weight} kg) `
                          : ""}
                        en {completedRun.duration} min
                      </span>
                      {completedRun.avg_bpm && (
                        <span
                          className={`${styles.bpmStat} ${completedRun.avg_bpm > 145 ? styles.bpmHigh : ""}`}
                        >
