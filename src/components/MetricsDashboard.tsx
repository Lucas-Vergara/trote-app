...
const ProgressRing = ({
  ...
  const safePct = isNaN(percentage)
    ? 0
  ...
  return (
    <div className={styles.progressRingWrapper}>
      ...
      <div className={styles.progressText}>{Math.round(safePct)}%</div>
    </div>
  );
};
...
export default function MetricsDashboard({
  ...
  const [activeTab, setActiveTab] = useState<"semanal" | "mensual" | "anual">(
    "semanal",
  );
  ...
  const troteRuns = useMemo(
    () => filteredRunsForStats.filter((r) => r.type !== "rucking"),
    [filteredRunsForStats],
  );
  const ruckingRuns = useMemo(
    () => filteredRunsForStats.filter((r) => r.type === "rucking"),
    [filteredRunsForStats],
  );

  // --- Km Totales ---
  const totalDistanceCombined = useMemo(
    () =>
      filteredRunsForStats.reduce(
        (sum, run) => sum + Number(run.distance || 0),
        0,
      ),
    [filteredRunsForStats],
  );
  const totalDistanceTrote = useMemo(
    () => troteRuns.reduce((sum, run) => sum + Number(run.distance || 0), 0),
    [troteRuns],
  );
  const totalDistanceRucking = useMemo(
    () => ruckingRuns.reduce((sum, run) => sum + Number(run.distance || 0), 0),
    [ruckingRuns],
  );
  ...
  const troteDuration = useMemo(
    () => troteRuns.reduce((sum, run) => sum + run.duration, 0),
    [troteRuns],
  );
  const avgPaceRaw =
    totalDistanceTrote > 0 ? troteDuration / totalDistanceTrote : 0;
  const avgPaceMinutes = Math.floor(avgPaceRaw);
  const avgPaceSeconds = Math.round((avgPaceRaw - avgPaceMinutes) * 60);
  const trotePaceStr =
    totalDistanceTrote > 0
      ? `${avgPaceMinutes}:${avgPaceSeconds.toString().padStart(2, "0")} min/km`
      : "0:00 min/km";

  // Rucking Average Weight
  const ruckingRunsWithWeight = useMemo(
    () =>
      ruckingRuns.filter(
        (r) => r.rucking_weight !== null && r.rucking_weight !== undefined,
      ),
    [ruckingRuns],
  );
  const totalRuckingWeight = useMemo(
    () =>
      ruckingRunsWithWeight.reduce(
        (sum, r) => sum + Number(r.rucking_weight),
        0,
      ),
    [ruckingRunsWithWeight],
  );
  const avgRuckingWeight =
    ruckingRunsWithWeight.length > 0
      ? totalRuckingWeight / ruckingRunsWithWeight.length
      : 0;
  const ruckingWeightStr =
    avgRuckingWeight > 0 ? `${avgRuckingWeight.toFixed(1)} kg` : "0.0 kg";

  // --- Tiempo Total ---
  const totalDurationCombined = useMemo(
    () => filteredRunsForStats.reduce((sum, run) => sum + run.duration, 0),
    [filteredRunsForStats],
  );
  const totalDurationTrote = useMemo(
    () => troteRuns.reduce((sum, run) => sum + run.duration, 0),
    [troteRuns],
  );
  const totalDurationRucking = useMemo(
    () => ruckingRuns.reduce((sum, run) => sum + run.duration, 0),
    [ruckingRuns],
  );
  ...
  const renderRendimientoValue = () => {
    if (totalDistanceTrote > 0 && avgRuckingWeight > 0) {
      ...
      return (
        <>
          {avgRuckingWeight.toFixed(1)}{" "}
          <span className={styles.boxUnit}>kg</span>
        </>
      );
    } else {
      return "0:00";
  ...
  const weekDistPct = weeklyGoal
    ? (weeklyDistance / weeklyGoal.target_distance) * 100
    : 0;
  const weekRunsPct = weeklyGoal
    ? (weeklyCount / weeklyGoal.target_runs) * 100
    : 0;
  const weekOverallPct = weeklyGoal ? (weekDistPct + weekRunsPct) / 2 : 0;

  const monthDistPct = monthlyGoal
    ? (monthlyDistance / monthlyGoal.target_distance) * 100
    : 0;
  const monthRunsPct = monthlyGoal
    ? (monthlyCount / monthlyGoal.target_runs) * 100
    : 0;
    ...
    <div className={styles.dashboardContainer}>
      ...
      {(showOnly === "all" || showOnly === "goals") && (
        <div className={styles.goalsGrid}>
                ...
                <div className={styles.goalStats}>
                  <div className={styles.statRow}>
                    ...
                    <span className={styles.statValue}>
                      ...
                      {weeklyGoal.target_distance} km
                    </span>
                  </div>
                  <div className={styles.progressBarBg}>
                    <div
                      className={`${styles.progressBarFill} ${styles.bgCyan}`}
                      style={{ width: `${Math.min(100, weekDistPct)}%` }}
                    ></div>
                  </div>
                  ...
                  <div className={styles.progressBarBg}>
                    <div
                      className={`${styles.progressBarFill} ${styles.bgCyan}`}
                      style={{ width: `${Math.min(100, weekRunsPct)}%` }}
                    ></div>
                  </div>
                </div>
              </div>
          ...
          {/* Meta Mensual */}
          <div
            className={`${styles.goalCard} glass-panel fade-in`}
              ...
              <div className={styles.goalBody}>
                ...
                <div className={styles.goalStats}>
                  <div className={styles.statRow}>
                    <span className={styles.statLabel}>Distancia:</span>
                    <span className={styles.statValue}>
                      <strong>{monthlyDistance.toFixed(1)}</strong> /{" "}
                      {monthlyGoal.target_distance} km
                    </span>
                  </div>
                  <div className={styles.progressBarBg}>
                    <div
                      className={`${styles.progressBarFill} ${styles.bgViolet}`}
                      style={{ width: `${Math.min(100, monthDistPct)}%` }}
                    ></div>
                  </div>
                  ...
                  <div className={styles.statRow}>
                    ...
                    <span className={styles.statValue}>
                      ...
                      {monthlyGoal.target_runs}
                    </span>
                  </div>
                  <div className={styles.progressBarBg}>
                    <div
                      className={`${styles.progressBarFill} ${styles.bgViolet}`}
                      style={{ width: `${Math.min(100, monthRunsPct)}%` }}
                    ></div>
                  </div>
                </div>
              </div>
      ...
      {(showOnly === "all" || showOnly === "stats") && (
        <div
          className={`${styles.statsCard} glass-panel fade-in`}
            ...
            <div className={styles.statBox}>
              ...
              <div className={styles.boxBreakdown}>
                <div className={styles.breakdownRow}>
                  <span className={styles.breakdownLabel}>🏃 Trote</span>
                  <span className={styles.breakdownValue}>
                    {formatDuration(totalDurationTrote)}
                  </span>
                </div>
                <div className={styles.breakdownRow}>
                  <span className={styles.breakdownLabel}>🎒 Rucking</span>
                  <span className={styles.breakdownValue}>
                    {formatDuration(totalDurationRucking)}
                  </span>
                </div>
              </div>
            </div>
