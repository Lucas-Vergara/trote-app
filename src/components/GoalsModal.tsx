...
export default function GoalsModal({
  ...
  useEffect(() => {
    if (existingGoal) {
      setDistance(existingGoal.target_distance.toString());
      setRuns(existingGoal.target_runs.toString());
    } else {
      setDistance('');
      setRuns('');
    }
      ...
      <div className={`${styles.modal} glass-panel fade-in`} onClick={e => e.stopPropagation()}>
        <div className={styles.modalHeader}>
          <h2 className={styles.modalTitle}>
            {existingGoal ? 'Editar Meta' : 'Crear Meta'} {type === 'weekly' ? 'Semanal' : 'Mensual'}
          </h2>
        ...
        <p className={styles.description}>
          Define tus objetivos de carrera. Calcularemos automáticamente el periodo de validez para esta {type === 'weekly' ? 'semana (Lunes a Domingo)' : 'mes completo'}.
        </p>
