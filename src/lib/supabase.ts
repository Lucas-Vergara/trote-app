...
const cleanSupabaseUrl = (url: string): string => {
  ...
  if (
    cleaned.includes("tu_supabase_project_url") ||
    cleaned.includes("placeholder-project")
  ) {
    return "";
  }
  if (
...
const cleanSupabaseKey = (key: string): string => {
  ...
  if (
    cleaned.includes("tu_supabase_anon_key") ||
    cleaned.includes("placeholder-anon-key")
  ) {
    return "";
  }
  return cleaned;
